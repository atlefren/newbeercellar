...
var Autocomplete = React.createClass({displayName: "Autocomplete",
    ...
    search: function () {
        ...
        //this.refs.input.getDOMElement().focus()
    ...
    render: function () {
        var input = (React.createElement(this.input, {ref: "input", value: this.state.searchVal, search: this.search}))
        var style = {display: "block"};
        if (!this.state.results.length) {
            var style = {display: "hidden"};
        }
        var breweryNodes = this.state.results.map(function (brewery) {
            return (
                React.createElement("li", null, React.createElement("a", {href: "/#"}, brewery.name))
            );
        });
